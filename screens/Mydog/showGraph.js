...
export default function showGraph({ navigation }) {
    ...
    useEffect(() => {
        const _retrieveData = async () => {
            try {
                const value = await AsyncStorage.getItem('udogid');
                if (value !== null) {
                    // We have data!!
                    setUdogid(value);
                }
                ...
                <View style={{ width: '100%', flexDirection: 'row' }}>
                    <ScrollView horizontal>
                        {week.map(item => (
                            ...
                            >
                                <Text style={{ color: active == item.content ? 'white' : '#555555', fontFamily: 'FC_Iconic', fontSize: 14 }}>{item.name}</Text>
                            </TouchableOpacity>
                        ))}
                    </ScrollView>
                </View>
                    ...
                    {list.map((item, k) => (
                        <>
                            <TouchableOpacity
                                ...
                                onPress={() => setActive(item.content)}
                            >
                                <Text style={{ color: active == item.content ? 'white' : '#555555', fontFamily: 'FC_Iconic', fontSize: 16 }}>{item.name}</Text>
                            </TouchableOpacity>
        ...
        <>
            ...
            <View style={styles.header}>
                <View style={{ width: '45%', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={{ marginLeft: 15, fontSize: 27, color: 'white', fontFamily: 'FC_Iconic' }}>{udogid}พัฒนาการโดยรวม</Text>
                </View>
